...
export interface PublicProgramBundle { program: Program; sessions: SessionRecord[]; }
...
export async function getPublicPrograms(): Promise<PublicProgramBundle[]> {
  ...
  const [programResult, sessionResult] = await Promise.all([
    supabase.from('public_programs').select('*').order('registration_deadline'),
    supabase.from('public_program_sessions').select('*').order('date').order('start_time'),
  ]);
  if (programResult.error || sessionResult.error) throw new DomainError('SERVICE_UNAVAILABLE');
  const programs = ((programResult.data ?? []) as Row[]).map(mapProgram);
  const sessions = ((sessionResult.data ?? []) as Row[]).map(mapSession);
  return programs.map((program) => ({ program, sessions: sessions.filter((session) => session.programId === program.id) }));
}
